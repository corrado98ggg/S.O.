...
int main(int argc, char **argv) 
{
	...
	int nro;		/* variabile per tenere traccia del numero di blocchi presenti nei file */
...
printf("Numero processi da creare %d con C=%d\n", 2*N, C);
...
for (i=0; i < 2*N; i++) 
{
	...
	if (pid == 0) 
	{
		...
		exit(nro/2); /* torniamo il numero di blocchi letti (supposto <= di 255) */
	}
}
