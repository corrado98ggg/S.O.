	 		...
	 		while (read(fd, &ch, 1)) 
			{
				if (ch == Cz) /* se abbiamo trovato il carattere da cercare */
				{
					...
					if (nr != sizeof(posLetta))
    					{		
                        			/* se non mi viene inviato alcuna posizione vuole dire che l'altro processo della coppia NON ha trovato altre occorrenze e quindi si può terminare la lettura */
						break;
         		 		}		
